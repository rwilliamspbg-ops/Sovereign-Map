      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 md:space-y-8 custom-scrollbar">
        {loading ? (
          ...
          <>
            ...
            {/* Added Sources Section for Search Grounding Compliance */}
            {insight.sources && insight.sources.length > 0 && (
              <section className="space-y-3">
                <h3 className="text-slate-500 text-[8px] mono font-bold uppercase tracking-[0.2em] flex items-center gap-2">
                  <span className="w-1 h-3 bg-emerald-600"></span> Grounding Sources
                </h3>
                <div className="grid gap-2">
                  {insight.sources.map((source, idx) => (
                    <a 
                      ...
                      className="p-3 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl transition-all flex items-center justify-between group"
                    >
                      <span className="text-[10px] text-slate-300 font-medium truncate pr-4">{source.title || source.uri}</span>
                      <svg className="w-3 h-3 text-slate-500 group-hover:text-blue-400 transition-colors shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
